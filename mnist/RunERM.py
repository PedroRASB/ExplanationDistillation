...
def main(hparams):
    ...
    if hparams.colors!='1':
        class MNISTDatset(Dataset):
            def __init__(self, mode,bias,masks=True,confounding=False,source='../data/mnist/',
                         randomColor=False):
                self.images=[]
                if (mode=='train'):
                    if (not bias):
            ...
            def __getitem__(self,idx):
                ...
                if self.randomColor:
                    image[0]=image[0]*random.uniform(0,1)
                    image[1]=image[1]*random.uniform(0,1)
                    image[2]=image[2]*random.uniform(0,1)
        ...
        class MNISTDatset(Dataset):#COLORED
            def __init__(self, mode,bias,privileged=False,confounding=False,
                         source='../data/mnistColor/',masks=False,randomColor=False):
                self.images=[]
                if (mode=='train'):
    ...
    trainSet=MNISTDatset('train',masks=heat,bias=bias,randomColor=(hparams.randomColor=='1'))
    testSet=MNISTDatset('test',masks=heat,bias=bias,randomColor=(hparams.randomColor=='1'))
    valSet=MNISTDatset('val',masks=heat,bias=bias,randomColor=(hparams.randomColor=='1'))
    ...
    clean=net.returnBackbone()
    torch.save(clean,TrainedPath+hparams.name+'Clean.pt')
    print('Saved:', TrainedPath+hparams.name+'Clean.pt')
    ...
    testSet=MNISTDatset('test',masks=False,bias=True,randomColor=(hparams.randomColor=='1'))
    ...
    testSet=MNISTDatset('test',masks=False,bias=False,randomColor=(hparams.randomColor=='1'))
    testingLoader=Tdata.DataLoader(testSet,batch_size=4,shuffle=False,
                                   num_workers=1)
    if(hparams.train=='1'):
        trainer.test(dataloaders=testingLoader)
    ...
    testSet=MNISTDatset('test',masks=False,bias=True,confounding=True,
                        randomColor=(hparams.randomColor=='1'))
    testingLoader=Tdata.DataLoader(testSet,batch_size=4,shuffle=False,
                                   num_workers=1)
    if(hparams.train=='1'):
        trainer.test(dataloaders=testingLoader)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--randomColor', default='0')
    parser.add_argument('--colors', default='0')
