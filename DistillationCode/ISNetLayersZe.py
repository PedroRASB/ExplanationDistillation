...
class LRPOutput (nn.Module):
    def __init__(self,layer=None,multiple=False,
                 positive=False,rule='e',ignore=None,amplify=1,highest=False,
                 randomLogit=False,backwardCompat=False):
        super().__init__()
        if not backwardCompat:
            self.layer=layer
        else:
            self.layer=None
...
class LRPDenseLayer (nn.Module):
    ...
    def forward (self,X,R):
        ...
        #R is none if there is no classifier and we are at the last dense block
...
class LRPDenseNetClassifier(nn.Module):
    def __init__(self,model,e,rule,multiple,positive,ignore,selective=False,highest=False,
                 amplify=1,randomLogit=False,backwardCompat=False):
        ...
        if selective:
            ...
            self.Out=LRPOutput(layer=classifier,multiple=multiple,positive=positive,
                              ...
                              amplify=amplify,randomLogit=randomLogit,backwardCompat=backwardCompat)
...
class _LRPDenseNet(nn.ModuleDict):
    def __init__(self,DenseNet,e,rule,multiple,positive,ignore=None,Zb=True,selective=False,
                 ...
                 storeRelevance=False,FSP=False,randomLogit=False,backwardCompat=False):
        ...
        self.add_module('LRPFinalLayers',
                        LRPDenseNetClassifier(model=model,e=last_e,rule=rule,multiple=multiple,
                                             ...
                                             randomLogit=randomLogit,backwardCompat=backwardCompat))
        changeRule=0
        if composite:
...
class _LRPResNet (nn.ModuleDict):
    def __init__(self,network,e,Zb,rule,selective,highest,multiple,positive,ignore=None,
...
class LRPResNetFixUpBottleneck (nn.Module):
    def __init__(self,model,e,rule,backbone):
        #model: bottleneck
        super(LRPResNetFixUpBottleneck ,self).__init__()
        ...
        #check for downsampling:
        if 'downsample' in self.bottleneck:
            self.LRPDownsample=LRPConvReLU(layer=self.bottleneck['downsample'][0],
                                               e=e,
                                               rule=rule)
            
        self.LRPc3=LRPConvReLU(layer=self.bottleneck['conv3'][0],
                                   e=e,
                                   rule=rule)
        self.LRPc2=LRPConvReLU(layer=self.bottleneck['conv2'][0],
                                   e=e,
                                   rule=rule)
        self.LRPc1=LRPConvReLU(layer=self.bottleneck['conv1'][0],
                                   e=e,
                                   rule=rule)
    ...
    def forward(self,rK):
        if 'downsample' not in self.bottleneck:#identity shotcut
            RShortcut,R=self.LRPSum(rK,self.bottleneck['conv1'][1].input,
                                    self.bottleneck['conv3'][1].output*self.scale)
        else:
            RShortcut,R=self.LRPSum(rK,self.bottleneck['downsample'][1].output,
                                    self.bottleneck['conv3'][1].output*self.scale)
            RShortcut=self.LRPDownsample(RShortcut,
                                         aJ=self.bottleneck['downsample'][1].input,
                                         aK=self.bottleneck['downsample'][1].output)

        R=self.LRPc3 (R,aJ=self.bottleneck['conv3'][1].input,
                      aK=self.bottleneck['conv3'][1].output)
        
        R=self.LRPc2 (R,aJ=self.bottleneck['conv2'][1].input,
                      aK=self.bottleneck['conv2'][1].output)
        
        R=self.LRPc1 (R,aJ=self.bottleneck['conv1'][1].input,
                      aK=self.bottleneck['conv1'][1].output)

        R=R+RShortcut
        
        return R
    
class LRPResNetFixUpBasicBlock (nn.Module):
    def __init__(self,model,e,rule,backbone):
        #model: bottleneck
        super(LRPResNetFixUpBasicBlock ,self).__init__()
        ...
        if (list(self.block.keys())[0]=='conv1'):#conv bn relu
            if 'downsample' in self.block:
                self.LRPDownsample=LRPConvReLU(layer=self.block['downsample'][0],
                                                   e=e,
                                                   rule=rule)
            ...
            self.LRPc2=LRPConvReLU(layer=self.block['conv2'][0],
                                       e=e,
                                       rule=rule)
            self.LRPc1=LRPConvReLU(layer=self.block['conv1'][0],
                                       e=e,
                                       rule=rule)
        else:
            raise ValueError('Unrecognized block configuration')
    ...
    def forward(self,rK):
        ...
        if 'downsample' not in self.block:#identity shotcut
            RShortcut,R=self.LRPSum(rK,self.block['conv1'][1].input,
                                    self.block['conv2'][1].output*self.scale)
        else:
            RShortcut,R=self.LRPSum(rK,self.block['downsample'][1].output,
                                    self.block['conv2'][1].output*self.scale)
            RShortcut=self.LRPDownsample(RShortcut,
                                         aJ=self.block['downsample'][1].input,
                                         aK=self.block['downsample'][1].output)
        ...
        R=self.LRPc1 (R,aJ=self.block['conv1'][1].input,
                      aK=self.block['conv1'][1].output)

            
        R=R+RShortcut
        return R
            
class _LRPResNetFixUpLayer (nn.ModuleDict):
    def __init__(self,model,idx,e,rule,backbone):
        #idx: index of the resnet layer
        super(_LRPResNetFixUpLayer ,self).__init__()
        layer=model['layer'+str(idx)]
        backboneLayer=getattr(backbone,'layer'+str(idx))
        bottle=False
        if 'conv3' in list(layer['0'].keys()):
            bottle=True
        ...
        for bottleneck in reversed(list(layer.keys())):
            if bottle:
                self.add_module('LRPBottleneck'+bottleneck,
                                LRPResNetFixUpBottleneck(layer[bottleneck],e,rule,
                                                         backboneLayer[int(bottleneck)]))
            else:
                self.add_module('LRPBasicBlock'+bottleneck,
                                LRPResNetFixUpBasicBlock(layer[bottleneck],e,rule,
                                                         backboneLayer[int(bottleneck)]))
...
class _LRPResNetFixUpBackbone (nn.ModuleDict):
    def __init__(self,model,e,Zb,rule,network,amplify=1):
        super(_LRPResNetFixUpBackbone ,self).__init__()
        ...
        for layer in reversed(list(backbone.keys())):
            if ('layer' in layer and 'fa_layer' not in layer):
                self.add_module('LRP'+layer,
                                _LRPResNetFixUpLayer(model,int(layer[-1]),e,rule,backbone=network))
        ...
        if not Zb:
            self.add_module('LRPFirstConv',
                            LRPConvReLU(layer=backbone['conv1'][0],
                                            e=e,
                                            rule=rule))
        else:
            self.add_module('LRPFirstConv',
                            ZbRuleConvInput(layer=backbone['conv1'][0],
                                              e=e,l=0,h=1))
    ...
    def forward(self,rK):
        ...
        rK=self.LRPFirstConv(rK,aJ=self.backbone['conv1'][1].input,
                             aK=self.backbone['conv1'][1].output)
        if self.amplify>1:
            rK=rK*self.amplify
...
class _LRPResNetFixUp (nn.ModuleDict):
    def __init__(self,network,e,Zb,rule,selective,highest,multiple,positive=False,ignore=None,
        ...
        self.add_module('LRPBackbone',
                        _LRPResNetFixUpBackbone(self.model,e,Zb,rule,network=network))
...
class LRPResNetFixUp (nn.Module):
    def __init__(self,network,heat,e,Zb,rule,selective,highest,multiple,positive,ignore=None,
                 amplify=1,detach=True,randomLogit=False):
        super(LRPResNetFixUp ,self).__init__()
        ...
        self.resnet=network
        self.LRPBlock=_LRPResNetFixUp(self.resnet,e,Zb,rule,selective,highest,multiple,positive,ignore,
                                amplify=amplify,detach=detach,randomLogit=randomLogit)
